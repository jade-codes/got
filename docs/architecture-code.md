...
│                                                                        │
│  TcpTransport (Transport impl)     Server: serve() + spawn_blocking   │
│    Noise NK over TcpStream           per-connection sync handler       │
│    16 MiB recv guard               Client: request_blocking / request  │
│                                      Noise NK initiate → exchange      │
│  Codec:                            FederationSyncManager:              │
│    encode/decode ExchangeRequest     async polling + RefreshPolicy     │
│    encode/decode ExchangeResponse    exponential backoff + staleness   │
...
got-core ─────────────────────────────────────────────────────────────
  ...
  │                                        │                        │
  │   got-net ── got-wire, got-core, reqwest, tokio                 │
...
| `reqwest 0.12` | got-web, got-net | HTTP client for LLM API relay (got-web) and federation sync (got-net) |
| `tokio` | got-net, got-web | Async runtime for server listener and spawn_blocking |
