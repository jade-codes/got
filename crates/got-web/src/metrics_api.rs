...
pub async fn coherence(
...
) -> Result<Json<CoherenceResponse>, (StatusCode, Json<crate::ApiError>)> {
    ...
    for c in &req.ordering {
        let dom = state.term_embeddings.get(&c.dominant)
            .ok_or_else(|| crate::api_err(StatusCode::BAD_REQUEST, format!("unknown term: {}", c.dominant)))?;
        let sub = state.term_embeddings.get(&c.subordinate)
            .ok_or_else(|| crate::api_err(StatusCode::BAD_REQUEST, format!("unknown term: {}", c.subordinate)))?;
    ...
    if constraints.is_empty() {
        return Err(crate::api_err(StatusCode::BAD_REQUEST, "no valid constraints"));
    }
    ...
    for (pos, h) in hidden_states.iter().enumerate() {
        let score = coherence_score(h, &ordering, &state.geometry, req.sharpness)
            .map_err(|e| crate::api_err(StatusCode::INTERNAL_SERVER_ERROR, format!("coherence: {e}")))?;
...
pub async fn collapse(
...
) -> Result<Json<CollapseResponse>, (StatusCode, Json<crate::ApiError>)> {
    ...
    if weights.is_empty() {
        return Err(crate::api_err(StatusCode::BAD_REQUEST, "no valid probe terms found"));
    }

    let weight_refs: Vec<&[f32]> = weights.iter().map(|w| w.as_slice()).collect();

    let proj = state.geometry.value_projected_gram(&weight_refs)
        .map_err(|e| crate::api_err(StatusCode::INTERNAL_SERVER_ERROR, format!("collapse: {e}")))?;
...
pub async fn compare(
...
) -> Result<Json<CompareResponse>, (StatusCode, Json<crate::ApiError>)> {
    ...
    let data = std::fs::read(&req.comparison_gotue_path)
        .map_err(|e| crate::api_err(StatusCode::BAD_REQUEST, format!("cannot read {}: {e}", req.comparison_gotue_path)))?;

    if data.len() < 14 || &data[0..4] != b"GOTU" {
        return Err(crate::api_err(StatusCode::BAD_REQUEST, "not a valid .gotue file"));
    }
    ...
    if offset + total * 4 > data.len() {
        return Err(crate::api_err(StatusCode::BAD_REQUEST, "gotue file truncated"));
    }
    ...
    let matrix_b = UnembeddingMatrix::new(vocab_size, hidden_dim, values)
        .map_err(|e| crate::api_err(StatusCode::BAD_REQUEST, format!("invalid gotue: {e}")))?;
    ...
    let dist = value_alignment_distance(
        ...
        &geo_b,
        probes.as_deref(),
    ).map_err(|e| crate::api_err(StatusCode::INTERNAL_SERVER_ERROR, format!("compare: {e}")))?;
